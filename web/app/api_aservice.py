...
def render_main_page(ES, tracer, days_ago, filter_text):
    ...
    # get all tracebacks
    with tracer.start_span('get all tracebacks', child_of=root_span):
    ...
    with tracer.start_span('determine hidden tracebacks', child_of=root_span):
    ...
    with tracer.start_span('get matching jira issues', child_of=root_span):
        for tb in tb_meta:
    ...
    with tracer.start_span('for each traceback, get similar tracebacks ', child_of=root_span):
        for tb in tb_meta:
            tb.similar_tracebacks = traceback_database.get_matching_tracebacks(
                ES, tb.traceback.traceback_text, es_util.EXACT_MATCH, 100
            )

    with tracer.start_span('render page', child_of=root_span):
